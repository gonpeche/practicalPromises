...
utils.writeFile = function (filename, data, callback) {
	var randExtraTime = Math.random() * 200;
	setTimeout(function () {
		fs.writeFile(filename, data, 'utf8', function (err, buffer) {
			if (err) callback(err);
			else callback(null, buffer.toString());
		});
	}, randExtraTime);
...
utils.promisifiedWriteFile = function (filename) {
	return new Promise(function (resolve, reject) {
		utils.readFile(filename, data, 'utf8', function (err, str) {
			if (err) reject(err);
			else resolve(str);
		});
	});
